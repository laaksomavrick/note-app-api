 ...
 */
export const createUser = async (db: Database, { email, password }: InsertUser): Promise<number> => {
    const { rows } = await db.query(`INSERT INTO users (email, password) VALUES ($1, $2) RETURNING id`, [
        email,
        password,
    ]);
