import { Database, getIdFromRows, parseRowsToType, RecordBase, updateTemplateStringAndArgs } from "../db";
 ...
 * Retrieves all the notes belonging to a particular folder.
 */
export const getNotesForFolder = async (db: Database, folderId: number): Promise<Note[]> => {
 ...
 */
export const createNote = async (db: Database, { name, content, userId, folderId }: NoteInput): Promise<Note> => {
