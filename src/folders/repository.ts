 ...
 */
export const createFolder = async (db: Database, { name, userId }: FolderInput): Promise<Folder> => {
    const { rows } = await db.query(`INSERT INTO folders (name, user_id) VALUES ($1, $2) RETURNING id`, [name, userId]);
 ...
 */
export const getFoldersForUser = async (db: Database, userId: number): Promise<Folder[] | null> => {
    const { rows } = await db.query(`SELECT * FROM folders WHERE user_id = $1 ORDER BY id`, [userId]);
 ...
 */
export const getFoldersForUserWithNotes = async (db: Database, userId: number): Promise<Folder[] | null> => {
...
export const updateFolder = async (
    ...
    const { rows } = await db.query(`UPDATE folders SET name = $1 WHERE id = $2 RETURNING id`, [name, folderId]);
    const id = getIdFromRows(rows);
    return this.findFolder(db, id);
};
